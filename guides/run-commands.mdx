...
Pass `timeoutSeconds` (TypeScript/Rust) or `timeout_seconds` (Python) to set a maximum wall-clock limit for the command.
...
<Note>
  When `timeoutSeconds` expires, FissionPlane terminates the process and throws a command-timeout error. Catch it with `CommandTimeoutError` (TypeScript/Python) or match on `fissionplane::Error` (Rust) to handle timeouts gracefully in your application logic.
</Note>
