---
title: "How FissionPlane Works: Four-Plane Architecture"
...
The node runtime (`vm-host`) runs as a privileged process on each sandbox node. It is the only component that creates, pauses, resumes, and destroys Firecracker microVMs. It manages per-sandbox networking, resource limits, and disk state. It also re-verifies capability tokens when traffic arrives from the gateway, and strips all platform credentials before anything reaches your code inside the sandbox.

Each `vm-host` is the authority on its own node's capacity. Replicas of the control plane never need to lock against each other to make placement decisions — they simply ask the node.
...
The control plane and gateway run as ordinary Kubernetes Deployments. The node runtime runs as a privileged DaemonSet on the labelled sandbox nodes. Kubernetes sees the control plane and gateway pods; it does not see individual microVMs — those are managed entirely by the node runtime below Kubernetes' visibility.
