---
title: "Get started with FissionPlane in minutes"
...
<Steps>
  ...
  <Step title="Create a sandbox">
    ...
    <Note>
      Sandboxes are deleted automatically when their deadline expires. The `deadline_seconds` value above gives the sandbox ten minutes. You can extend the deadline at any time with `sandbox.extendDeadline()` (TypeScript), `sandbox.extend_deadline()` (Python), or `sandbox.extend_deadline()` (Rust).
    </Note>
