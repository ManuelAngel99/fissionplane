...
All template endpoints are on the control plane: `https://<your-domain>/v1`. Authenticate with your organisation API key or OIDC token in the `Authorization: Bearer` header.

<Note>
  Builds are asynchronous. After you start a build with `POST /v1/templates/builds`,
  poll `GET /v1/templates/builds/{id}` for status and tail output with
  `GET /v1/templates/builds/{id}/logs` until the build reaches a terminal state.
</Note>
...
## GET /v1/templates/builds/{id}/logs

Returns log entries from a template build starting at `offset`. Poll this endpoint alongside `GET /v1/templates/builds/{id}` to tail build output. A call at the current end of the log returns an empty page and is not an error.
